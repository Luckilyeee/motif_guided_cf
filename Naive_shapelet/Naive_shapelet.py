...
class Naive_Shapelet:
    ...
    def generate_cf(self):
        ...
        for i in range(nb_classes):
            plot_shapelet(self.X_train, shapelets, i)
        ...
        return accuracy, model, targets

    def plot_comparison_res(self, index):
        counterfactual_examples = np.load(self.DS_name + '_' + self.method +'_cf.npy')
        plot_cf_vs_ori(self.DS_name, index, counterfactual_examples)

    def save_res(self):
        accuracy, model, targets = self.generate_cf()
        counterfactual_examples = np.load(self.DS_name + '_' + self.method +'_cf.npy')
